...
export class LanguageService {
  ...
  private dictionary: { [key: string]: { [key in Language]: string } } = {
    'app.title': {
      zho: 'National Capital Card',
      eng: 'National Capital Card',
      jpn: 'National Capital Card',
    },
    'app.subtitle': {
      zho: '探索世界各國，挑戰你的地理知識',
      eng: 'Explore the world, challenge your geography knowledge',
      jpn: '世界を探索し、地理の知識に挑戦しよう',
    },
    ...
    'q.continent': {
      zho: '位於哪一個洲？',
      eng: 'is located in which continent?',
      jpn: 'はどの大陸にありますか？',
    },
    'q.reverse_flag': {
      zho: '哪一面是',
      eng: 'Which is the flag of',
      jpn: 'どちらが',
    },
    'q.reverse_flag_suffix': {
      zho: '的國旗？',
      eng: '?',
      jpn: 'の国旗ですか？',
    },
    ...
    'view.card': { zho: '卡牌', eng: 'Card', jpn: 'カード' },
    'view.list': { zho: '列表', eng: 'List', jpn: 'リスト' },
    'sort.label': { zho: '排序', eng: 'Sort', jpn: '並べ替え' },
    'sort.name': { zho: '名稱', eng: 'Name', jpn: '名前' },
    'sort.population': { zho: '人口', eng: 'Population', jpn: '人口' },
  ...
  getCountryName(country: any): string {
    const lang = this.getCurrentLanguage();
    if (lang === 'zho')
      return country.translations?.zho?.common || country.name.common;
    if (lang === 'jpn')
